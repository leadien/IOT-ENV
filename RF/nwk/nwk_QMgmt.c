...
Revised:        $Date: 2008-01-06 19:19:46 -0800 (Sun, 06 Jan 2008) $
Revision:       $Revision: 16152 $
...
void nwk_QadjustOrder(uint8_t which, uint16_t stamp)
{
	frameInfo_t *pFI;
	//uint8_t      i, num;
	uint16_t      i, num;
...
* input parameters
* @param   which   - INQ or OUTQ to adjust
* @param   port    - port on whcih to look
...
{
	//uint8_t      i, oldest, num, port,srcport;
	...
	for (i=0; i<num; ++i, ++wPtr)
	{
		...
		if (uType == wPtr->fi_usage)
		{
			...
			srcport = GET_FROM_FRAME(MRFI_P_PAYLOAD(&wPtr->mrfiPkt), F_PORT_OS);
			if (srcport == port || SMPL_PORT_CALL == srcport )
			{
