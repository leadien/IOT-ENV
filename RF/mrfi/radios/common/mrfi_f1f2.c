...
uint8_t MRFI_SetRxAddrFilter(uint8_t * pAddr)
{
  ...
  sio_printf("\n	hardware addr: %02x\n",RF_ED);
  //MRFI_WRITE_REGISTER( ADDR, RF_ED );
...
//added by cbwang
/**************************************************************************************************
 * @fn          MRFI_CheckSystemID
 *
 * @brief       Determine if address is filtered.
 *
 * @param       none
 *
 * @return      zero     : address is filtered
 *              non-zero : the same system id
 **************************************************************************************************
 */
extern UINT16 g_WERSSystemId;
uint8_t MRFI_CheckSameSystemID(uint8_t * pAddr)
{	
	if ( pAddr )
	{
		UINT16 systemID;
		
		memcpy(&systemID, pAddr, 2);
		
		if( systemID != g_WERSSystemId )
		{
		  	//printf_0("==CheckSystemID not the same system, (%d ,%d )\n",systemID,g_WERSSystemId);
			return (0);
		}
		else
		{
