...
* ------------------------------------------------------------------------------------------------
*/
#define MRFI_SPI_TURN_CHIP_SELECT_ON()        MRFI_SPI_DRIVE_CSN_LOW(); MRFI_SPI_WAIT_SO_IS_HIGH() 
...
#define MRFI_DELAY_CNT	(72 + 72 + 5)
#define MRFI_DELAY_TCNT	(72 + 72 + 5)
void mrfi_delay(uint32_t i)
{
	while(i)i--;
}
/**************************************************************************************************
...
uint8_t mrfiSpiReadReg(uint8_t addr)
{
	...
	*/
	return( spiRegAccess(addr | MRFI_BURST_BIT | MRFI_READ_BIT, MRFI_DUMMY_BYTE) );
}
...
{
	spiBurstFifoAccess(TXFIFO | MRFI_BURST_BIT, pData, len);
}
...
{
	spiBurstFifoAccess(RXFIFO | MRFI_BURST_BIT | MRFI_READ_BIT, pData, len);
}
