...
10	0xC0	28.8
*/
#define MRFI_SETTING_PATABLE0       0xC0 //0x51


#elif (defined MRFI_CC1101)
#define MRFI_RADIO_PARTNUM          0x00
#define MRFI_RADIO_MIN_VERSION      4
#define MRFI_SETTING_PATABLE0       0xC0//0x3F//0x51
...
*/
#if 0 /*changed by cbwang@20080603*/
#define MRFI_MAX_DELAY_US 16 /* usec */
#else
#define MRFI_MAX_DELAY_US 1 /* usec */
#endif
...
* ------------------------------------------------------------------------------------------------
*/
//for cc1100 module test
#define MRFI_CONFIG_TX_EN_AS_OUTPUT()	MRFI_TX_EN_PIN_AS_OUPUT()
#define MRFI_CONFIG_RX_EN_AS_OUTPUT()	MRFI_RX_EN_PIN_AS_OUPUT()
#define MRFI_ENABLE_RX_EN_PIN()			MRFI_RX_EN_HIGH()
#define MRFI_DISABLE_RX_EN_PIN()		MRFI_RX_EN_LOW()
#define MRFI_ENABLE_TX_EN_PIN()			MRFI_TX_EN_HIGH()
#define MRFI_DISABLE_TX_EN_PIN()		MRFI_TX_EN_LOW()
//
...
{                                                                             \
	int16_t delay = MRFI_BOARD_RSSI_VALID_DELAY_US;                             \
	do                                                                          \
  {                                                                           \
...
static const uint8_t mrfiRadioCfg[][2] =
{
	...
	{  PA_TABLE0, /*0xC0*/ MRFI_SETTING_PATABLE0  },
#ifdef MRFI_CC1101
...
volatile uint32_t	g_intr_idle = 0;

int8_t g_mrfi_rssi = 0;
int8_t g_mrfi_LQI = 0;

UINT32  g_CRCfailedPacketNumber = 0;
UINT32  g_MismatchLengthNumber  = 0;
UINT32 g_ReceiveNotEmptyPacketNumber = 0;

volatile uint32_t	g_intr_idle;
/* ------------------------------------------------------------------------------------------------
*                                       Local Prototypes
* ------------------------------------------------------------------------------------------------
*/
...
* ------------------------------------------------------------------------------------------------
*/
volatile static uint8_t mrfiRadioState  = MRFI_RADIO_STATE_UNKNOWN;
static mrfiPacket_t mrfiIncomingPacket;
/*changed by cbwang@20080603*/
/*__no_init static uint8_t mrfiRndSeed;*/
...
#define TEST_VALUE 0xA5
	printf_0(" VERSION value: %x\n", mrfiSpiReadReg( VERSION ));
	printf_0("0 PKTLEN value: %x\n", mrfiSpiReadReg( PKTLEN ));
	mrfiSpiWriteReg( PKTLEN, TEST_VALUE );
	printf_0("1 PKTLEN value: %x\n", mrfiSpiReadReg( PKTLEN ));
...
#ifdef _CC1100_RF_MODULE_
	//MRFI_CONFIG_TX_EN_AS_OUTPUT();
	//MRFI_CONFIG_RX_EN_AS_OUTPUT();
	MRFI_ENABLE_RX_EN_PIN();
	MRFI_DISABLE_TX_EN_PIN();
	//MRFI_DISABLE_RX_EN_PIN();
#endif
...
#else
				for(count = 72*1000*5; count; count--)if(MRFI_PAPD_PIN_IS_HIGH())break;
				if(!count)
				{
					//dbgcnt ++;
					//if(dbgcnt >1)
...
void MRFI_Receive(mrfiPacket_t * pPacket)
{
	/* modified by cbwang */
#if 1//0
	memcpy(pPacket,&mrfiIncomingPacket,sizeof(mrfiPacket_t));
#else
		...
		*/
		...
		else
		{
			...
			*/
			...
			{
				UINT32 i;
				UINT8 *pBuffer = &(mrfiIncomingPacket.frame[MRFI_FRAME_BODY_OFS]);
			...
			else
			{
				...
				*/
				...
				if ( ( !MRFI_RxAddrIsFiltered(MRFI_P_DST_ADDR(&mrfiIncomingPacket)) )
					&& MRFI_CheckSameSystemID(MRFI_P_SRC_ADDR(&mrfiIncomingPacket)) )
				{
					{
						/* ------------------------------------------------------------------
						*    Receive successful
...
static void Mrfi_RxModeOff(void)
{
	...
	mrfiSpiCmdStrobe( SIDLE );
	//	while (mrfiSpiCmdStrobe( SNOP ) & 0xF0);
	//while (mrfiSpiCmdStrobe( SNOP ) & 0xE0);
#endif
	/* flush the receive FIFO of any residual data */
	mrfiSpiCmdStrobe( SFTX );
	mrfiSpiCmdStrobe( SFRX );
...
**************************************************************************************************
*/
extern void SPMP_TurnLED(UINT8 led, UINT32 flag); 
...
void MRFI_GpioIsr(void)
{
	...
	if (MRFI_SYNC_PIN_INT_IS_ENABLED() /*&& MRFI_SYNC_PIN_INT_FLAG_IS_SET()*/) //chhanged by cbwang@20080618
	{
...
int8_t Mrfi_CalculateRssi(uint8_t rawValue)
{
	...
	*/
	if(rawValue >= 128)
	{
		rssi = (int16_t)(rawValue - 256)/2 - MRFI_BOARD_RSSI_OFFSET;
	}
	else
	{
		rssi = (rawValue/2) - MRFI_BOARD_RSSI_OFFSET;
	}
...
void MRFI_DelayMs(uint16_t milliseconds)
{
	...
	//bspIState_t s;
	//BSP_ENTER_CRITICAL_SECTION(s);
	//hwWait(0, milliseconds);
	BSP_DELAY_MSECS(milliseconds);
	//BSP_EXIT_CRITICAL_SECTION(s);
#endif
}
